...
// 1139 {
// 1140   //根据ftmx禁用相应pin的ftm功能，回复到默认功能
// 1141   if(ftmx == FTM0)
LPLD_FTM_PinDeinit:
...
// 1343   ftmx->MODE |= FTM_MODE_FTMEN_MASK;    //FTM2EN=1   
        ...
        STR      R2,[R1, #+84]
// 1344   
// 1345   ftmx->CNTIN = 0;//FTM0计数器初始值为0  
        MOVS     R2,#+0
        STR      R2,[R1, #+76]
// 1346   ftmx->CNT=0;  
        MOVS     R2,#+0
        STR      R2,[R1, #+4]
// 1347   ftmx->MOD = 0xFFFF;//结束值  
        MOVW     R2,#+65535
        STR      R2,[R1, #+8]
// 1348   
// 1349   ftmx->QDCTRL |= mode;     //解码模式选择
        LDR      R2,[R1, #+128]
        UXTB     R0,R0            ;; ZeroExt  R0,R0,#+24,#+24
        ORRS     R0,R0,R2
        STR      R0,[R1, #+128]
// 1350   ftmx->QDCTRL |= FTM_QDCTRL_QUADEN_MASK;       //使能正交解码模式 
        LDR      R0,[R1, #+128]
        ORRS     R0,R0,#0x1
        STR      R0,[R1, #+128]
// 1351 
...
// 1360 
// 1361 /*
// 1362  * FTM0--FTM1中断处理函数
// 1363  * 与启动文件startup_K60.s中的中断向量表关联
// 1364  * 用户无需修改，程序自动进入对应通道中断函数
// 1365  */
...
// 1370   OS_ENTER_CRITICAL(); //告知系统此时已经进入了中断服务子函数
// 1371   OSIntEnter();
...
// 1376   
// 1377 #if (UCOS_II > 0u)
// 1378   OSIntExit();          //告知系统此时即将离开中断服务子函数
// 1379 #endif
// 1380 }
        POP      {R0,PC}          ;; return
// 1381 
...
// 1386   OS_ENTER_CRITICAL(); //告知系统此时已经进入了中断服务子函数
// 1387   OSIntEnter();
...
// 1392   
// 1393 #if (UCOS_II > 0u)
// 1394   OSIntExit();          //告知系统此时即将离开中断服务子函数
// 1395 #endif
// 1396 }
        POP      {R0,PC}          ;; return
// 1397 
...
// 1402   OS_ENTER_CRITICAL(); //告知系统此时已经进入了中断服务子函数
// 1403   OSIntEnter();
...
// 1408   
// 1409 #if (UCOS_II > 0u)
// 1410   OSIntExit();          //告知系统此时即将离开中断服务子函数
// 1411 #endif
// 1412 }
        POP      {R0,PC}          ;; return
...
// 1419   OS_ENTER_CRITICAL(); //告知系统此时已经进入了中断服务子函数
// 1420   OSIntEnter();
...
// 1425   
// 1426 #if (UCOS_II > 0u)
// 1427   OSIntExit();          //告知系统此时即将离开中断服务子函数
// 1428 #endif
// 1429 }
// 1430 #endif
// 
